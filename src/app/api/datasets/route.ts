...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user.isPaid) {
      ...
      if (count >= 3) {
        return NextResponse.json({ message: 'Free tier limit reached (Max 3 Datasets). Please upgrade.' }, { status: 400 });
      }
      
      // Row Limit
      if (Array.isArray(data) && data.length > 5000) {
        return NextResponse.json({ message: 'Free tier limit reached (Max 5,000 rows). Please upgrade.' }, { status: 400 });
        }
    }
