...
export default function NewAnalysisWizard() {
    ...
    if (uploadedFile) {
      ...
      reader.onload = (evt) => {
        try {
          ...
          const jsonData = XLSX.utils.sheet_to_json(ws) as Record<
            string,
            unknown
          >[];
  ...
  const handleFinish = async () => {
    ...
    const promise = (async () => {
      ...
      if (!datasetRes.ok) throw new Error(dataset.message);
        ...
        <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
          ...
          {widgets.length === 0 ? (
            ...
            >
              ...
              <p>
                Click &quot;Add Widget&quot; to create your first chart or KPI
              </p>
            </div>
          ) : (
          ...
          <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
                      ...
                      {["sum", "avg", "count"].map((op) => (
                        <button
                          ...
                          onClick={() =>
                            setWidgetConfig({
                              ...widgetConfig,
                              operation: op as "sum" | "avg" | "count",
                            })
                          }
                          className={`flex-1 py-1 text-sm font-medium rounded capitalize ${
                            widgetConfig.operation === op
              ...
              <div>
                ...
                <div className="flex gap-3">
                  {["#3b82f6", "#8b5cf6", "#f43f5e", "#10b981", "#f59e0b"].map(
                    (c) => (
                      <button
                        key={c}
                        onClick={() =>
                          setWidgetConfig({ ...widgetConfig, color: c })
                        }
                        className={`w-8 h-8 rounded-full border-2 transition-transform hover:scale-110 ${
                          widgetConfig.color === c
            ...
            <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-800">
              ...
              <button onClick={addWidgetToList} className="btn-primary">
                Add Widget
              </button>
            </div>
          </div>
